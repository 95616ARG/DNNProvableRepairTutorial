...
class Module(nn.Module):
    ...
    def parameter_deltas(self, concat: bool=True) -> Generator[SymbolicArray] | SymbolicArray:
    ...
    def delta(self, points, sym_output=None, norm='linf+l1_normalized'):
        with no_symbolic():
            ref_output = self(points)
        if sym_output is None:
            sym_output = self(points)
        output_deltas = (sym_output - ref_output).flatten()
        param_deltas = self.parameter_deltas()
        return output_deltas.norm_ub(norm) + param_deltas.norm_ub(norm)
